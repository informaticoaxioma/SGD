...
if (!empty($_POST['usuarios'])) {
  ...
  $id_selectC=$_POST['usuarios'];

$fil = implode(",", $_POST['usuarios']);
$filtraContrato= "AND u.id_usuario IN (".$fil.")";
...
$query5= "SELECT u.id_usuario,UPPER(CONCAT(u.nombre,' ',u.apellido_p, ' ',u.apellido_m)) AS nombre FROM usuario u WHERE u.id_estado_usuario=1 AND u.id_contrato=18 ORDER BY nombre ASC;  ";

$contratos5 = $_conexion5->obtenerDatos($query5);

$_conexion1 = new conexion;
$query1= "SELECT u.id_usuario,UPPER(CONCAT(u.nombre,' ',u.apellido_p, ' ',u.apellido_m)) AS nombre FROM usuario u WHERE u.id_estado_usuario=1 AND u.id_contrato=18 ".$filtraContrato." ORDER BY nombre ASC ";
...
for ($n=0; $n < count($contratos); $n++) {
  $combreContratos.= " data.addColumn('number', '".$contratos[$n]['nombre']."'); ";
}
...
for ($c=0; $c < count($contratos); $c++) {
    ...
    $query3= "SELECT count(d.id_documento) AS total
      FROM documento d
      INNER JOIN detalle_documento de
      ON de.id_documento=d.id_documento
      WHERE YEAR(d.fecha_recepcion)=".$meses[$m]['year']." AND MONTH(d.fecha_recepcion)=".$meses[$m]['mes']."
      AND de.id_responsable=".$contratos[$c]['id_usuario'].";";
    ...
    $tab[]=array(
      'id_usuario' => $contratos[$c]['id_usuario'],
      'nombre' => $contratos[$c]['nombre'],
      'year' => $meses[$m]['year'],
      'mes' => $meses[$m]['mes'],
      'dato' => $total[0]['total']
    );
...
for ($cc=0; $cc < count($contratos); $cc++) {
  $cCs.= '<th>'.$contratos[$cc]['nombre'].'</th>';
}

$tabla = '<h4>Detalle de responsables por documentos en SGD</h4><br>
...
<html lang="en">
 ...
 <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
   ...
   function drawChart() {
     ...
     var options = {
       chart: {
         title: 'Registros de responsables de documentos en SGD',
  ...
  <body>
    ...
    <div class="container-fluid">
        ...
        <div class="col-md-4">
          ...
          <select class="js-example-basic-multiple form-select" name="usuarios[]" multiple="multiple">
            <?php
            for ($n=0; $n < count($contratos5); $n++) {
              (in_array($contratos5[$n]['id_usuario'],$id_selectC)) ? $select ='selected' : $select ='';

              echo '<option '.$select.' value="'.$contratos5[$n]['id_usuario'].'">'.$contratos5[$n]['nombre'].'</option>';
            }
             ?>
          </select>
        </div>
