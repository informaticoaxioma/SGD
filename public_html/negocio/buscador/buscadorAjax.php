...
switch ($id) {
	case '1':
        ...
        $fechaDocBuscar = htmlspecialchars($_POST['fechaDocBuscar']) != "" ? htmlspecialchars($_POST['fechaDocBuscar']) : '';
				$fechaDocBuscar2 = htmlspecialchars($_POST['fechaDocBuscar2']) != "" ? htmlspecialchars($_POST['fechaDocBuscar2']) : '';
        ...
        $idEstadoDoc = htmlspecialchars($_POST['idEstadoDoc']) != "" ? htmlspecialchars($_POST['idEstadoDoc']) : '';
        $idSubContrato = htmlspecialchars($_POST['idSubContrato']) != "" ? htmlspecialchars($_POST['idSubContrato']) : '';
        $idCont = htmlspecialchars($_POST['idContrato']) != "" ? htmlspecialchars($_POST['idContrato']) : '';

        $documento->setFechaDocumento($fechaDocBuscar != "" ? $serviceDocumento->formatoFechaGuardarDB($fechaDocBuscar) : '');
				$documento->setFechaDocumento2($fechaDocBuscar2 != "" ? $serviceDocumento->formatoFechaGuardarDB($fechaDocBuscar2) : '');
        ...
        $documento->setIdContrato($idCont);
        ...
        $documentos = $serviceDocumento->resultadoBuscador($documento, $detalleDoc, $sessionUsuario->getIdContrato(), $sessionUsuario->getIdPerfil(),$idSubContrato);
...
$fechaActual = date_create($d['fecha_plazo']);
$plazo = date_create(date("Y-m-d"));
$intervalFecha = date_diff($plazo, $fechaActual);
$salidaFecha = $intervalFecha->format('%R%a');
($salidaFecha <= 2 ) ? $styleF="rojo" : $styleF="";
            ...
            echo "<td>" . $d['fecha_documento'] . "</td>";
            echo "<td>" . $d['fecha_plazo'] . "</td>";
//	    echo "<td>";
		  if($d['fecha_plazo'] <> ''){
			//echo $salidaFecha;
			//var_dump($d['fecha_plazo']);
		}
  //          echo "</td>";
            ...
            echo "<td>";
            echo '<form action="../negocio/abrirDocumento.php" method="POST" target="_blank">';
            ...
            if ($d['nombre_documento'] != "") {//SI POSEE UN DOCUMENTO SE MUESTRA EL BOTON SINO SE DESPLIEGA UN MENSAJE
                echo '<button id = "btnDescargarArchivo" name = "btnDescargarArchivo" class = "btn btn-primary btn-sm" ><i class = "fa fa-eye"></i>&nbsp;
            Abrir</button>';
            } else {
            ...
            echo '</td>';
            echo '<td>';
            echo '<a href="buscador/detalleDocumentoBuscador.php?idDocumento=' . $d['id_documento'] . '" target="_blank" ><button id="btnDetalle" name="btnDetalle" class="btn btn-warning btn-sm" ><i class="fa fa-plus-circle"></i>&nbsp;Detalle</button></a>';
            echo '</td>';
            if ($sessionUsuario->getIdPerfil() == 1 || $sessionUsuario->getIdPerfil() == 3 || $sessionUsuario->getIdPerfil() == 9) :
                echo "<td>";
