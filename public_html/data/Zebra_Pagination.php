...
class Zebra_Pagination {
    ...
    private function _show_next() {
        ...
        // if we're on the last page, the link is disabled
            $output = '<li' . ($this->_properties['page'] == $this->_properties['total_pages'] ? ' class="disabled"' : '') . '><a class="siguiente" href="' .
                    ...
                    ' rel="next">' .
                    ($this->_properties['reverse'] ? $this->_properties['previous'] : $this->_properties['next']) . '<input type="hidden" value="' . $this->_properties['page'] . '"></a></li>';
    ...
    private function _show_pages() {
        ...
        if ($this->_properties['total_pages'] <= $this->_properties['selectable_pages']) {
            ...
            for (
            $i = ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1); ($this->_properties['reverse'] ? $i >= 1 : $i <= $this->_properties['total_pages']); ($this->_properties['reverse'] ? $i-- : $i++)
            )

            // render the link for each page making sure to highlight the currently selected page
                $output .= '<li' . ($this->_properties['page'] == $i ? ' class="active"' : '') . '><a class="aNext" href="' . $this->_build_uri($i) . '">' .
            ...
            $output .= '<li' . ($this->_properties['page'] == ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) ? ' class="active"' : '') . '><a class="aNext" href="' . $this->_build_uri($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) . '">' .
            ...
            $output .= '<li' . ($this->_properties['page'] == $i ? ' class="active"' : '') . '><a class="aNext" href="' . $this->_build_uri($this->_properties['reverse'] ? 1 : $this->_properties['total_pages']) . '">' .
    ...
    private function _show_previous() {
        ...
        // if we're on the first page, the link is disabled
            $output = '<li' . ($this->_properties['page'] == 1 ? ' class="disabled"' : '') . '><a class="aPagination" href="' .
